    ...
    <nav className="bg-white border-gray-200 dark:bg-gray-900  fixed z-50 w-full ">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        <NavLink to="" className="flex items-center space-x-3 rtl:space-x-reverse">
          <img src={img2} className="h-8" alt="Flowbite Logo" />
        ...
        <div className={`${isOpen ? "block" : "hidden"} w-full md:block md:w-auto`} id="navbar-default">
          <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 
            ...
            {["Home", "About", "Contact", "SignUp", "Login"].map((item, index) => (
              <li key={index} >
               
                <NavLink
                  onClick={()=>setPage(item)}
                  to={item == "SignUp" ||item == "Login"?`/auth/${item}`:`/${item}`}
                  className={({isActive})=>`block py-2 px-3 rounded-sm ${
                     isActive
